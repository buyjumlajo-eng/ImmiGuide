...
export const LetterGenerator: React.FC = () => {
  ...
  useEffect(() => {
      if (formData.formType.includes('I-130')) {
          setCategories([
              { category: "Proof of Petitioner's Status", items: ['Birth Certificate'] },
              { category: "Proof of Relationship", items: ['Marriage Certificate'] },
              { category: "Bona Fide Marriage Evidence", items: ['Joint Lease', 'Joint Bank Account Statement'] },
          ...
          setCategories([
               ...
               { category: "Proof of Meeting in Person", items: ['Airline Tickets', 'Photos together'] },
               { category: "Intent to Marry", items: ['Letters of Intent'] }
          ]);
      }
  }, [formData.formType]);
  ...
  return (
    <div className="max-w-5xl mx-auto animate-in fade-in duration-500 pb-20">
      <div className="text-center mb-10">
          ...
          <p className="text-slate-500 max-w-xl mx-auto">Create a professional, structured cover letter that links your evidence directly to USCIS requirements.</p>
      </div>
          ...
          <div className="bg-slate-50 border-r border-slate-100 p-8 w-full md:w-72 flex-shrink-0">
              <div className="space-y-8 relative">
                  ...
                  <div className={`relative z-10 flex items-center gap-4 ${step === 1 ? 'text-blue-600' : 'text-slate-500'}`}>
                      ...
                      <div>
                          ...
                          <span className="text-xs opacity-70">Parties & Filing Info</span>
                      </div>
                  </div>
                  ...
                  <div className={`relative z-10 flex items-center gap-4 ${step === 2 ? 'text-blue-600' : 'text-slate-500'}`}>
                      ...
                      <div>
                          <span className="font-bold text-sm block">Evidence Index</span>
                          <span className="text-xs opacity-70">Organize Exhibits</span>
                      </div>
                  </div>
              ...
              {step === 1 && (
                  <div className="mt-12 bg-blue-50 p-4 rounded-xl border border-blue-100 text-xs text-blue-800 leading-relaxed">
                      <strong>Tip:</strong> Accurate addresses and status details help the AI establish proper jurisdiction for your case.
                  </div>
              )}
          </div>
          ...
          <div className="flex-1 p-8 md:p-12 relative flex flex-col">
              {step === 1 && (
                  <div className="space-y-6 animate-in slide-in-from-right-4">
                      <h2 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
                          <Building className="w-6 h-6 text-slate-400" /> Filing Details
                      </h2>
                      
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="md:col-span-2">
                               <label className="block text-sm font-bold text-slate-700 mb-2">Petition Type</label>
                               <select 
                          ...
                          <div className="md:col-span-2">
                              <label className="block text-sm font-bold text-slate-700 mb-2">USCIS Filing Address / Service Center</label>
                              <input 
                                value={formData.serviceCenter}
                                onChange={(e) => setFormData(prev => ({...prev, serviceCenter: e.target.value}))}
                                placeholder="e.g. USCIS, Attn: I-130, P.O. Box 21700, Phoenix, AZ 85036"
                                className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition-all hover:bg-white"
                              />
                          </div>

                          <div>
                              <label className="block text-sm font-bold text-slate-700 mb-2">Petitioner Name</label>
                          ...
                          <div className="md:col-span-2">
                              <label className="block text-sm font-bold text-slate-700 mb-2">Beneficiary Name (Applicant)</label>
                              <div className="relative">
                                <User className="absolute left-3 top-3.5 w-4 h-4 text-slate-400" />
                                <input 
              ...
              {step === 2 && (
                   <div className="space-y-6 animate-in slide-in-from-right-4 flex-1 flex flex-col">
                      <div>
                        <h2 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
                            <Files className="w-6 h-6 text-slate-400" /> Evidence Index
                        </h2>
                        <p className="text-slate-500 text-sm mt-1">Group your documents by category. This structure creates the "Exhibit List" in your letter.</p>
                      </div>
                              ...
                              <div key={catIdx} className="bg-slate-50 rounded-xl p-4 border border-slate-200">
                                  <div className="flex justify-between items-center mb-3">
                                      <h3 className="font-bold text-slate-700 flex items-center gap-2">
                                          <span className="bg-slate-200 text-slate-600 text-xs px-2 py-0.5 rounded">Exhibit {String.fromCharCode(65 + catIdx)}</span>
                                          <input 
                                            value={cat.category}
                                            onChange={(e) => {
                                                const newCats = [...categories];
                                      ...
                                      {cat.items.map((item, itemIdx) => (
                                          <div key={itemIdx} className="flex gap-2">
                                              <input 
                                                ...
                                                placeholder="Document Description (e.g., Marriage Certificate)"
                  ...
                  <div className="space-y-4 animate-in slide-in-from-bottom-4 h-full flex flex-col">
                       <div className="flex justify-between items-center mb-2">
                            ...
                            <div className="flex gap-2">
                                <button className="p-2 hover:bg-slate-100 rounded-lg text-slate-600" title="Copy Text" onClick={() => navigator.clipboard.writeText(letter)}>
                                    <Copy className="w-4 h-4" />
                                </button>
                                <button className="p-2 hover:bg-slate-100 rounded-lg text-slate-600" title="Download .txt">
                       ...
                       <div className="flex-1 bg-white border border-slate-200 shadow-inner rounded-xl p-8 overflow-y-auto custom-scrollbar relative">
                        ...
                        <div className="flex justify-between pt-4">
                          ...
                          <button className="bg-green-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-green-700 shadow-lg shadow-green-200 flex items-center gap-2">
